...
const BasketballCourtSimulator = () => {
  ...
  const [selectedPlayer, setSelectedPlayer] = useState('generic');
  ...
  // Player shooting profiles based on NBA stats
  const playerProfiles = {
    'generic': {
      name: 'Generic Player',
      restricted: 64,  // NBA average 60-64% in restricted area
      paint: 43,       // NBA average ~40-43% in non-restricted paint (4-9 ft)
      midRange: 40,    // NBA average ~40% from 8-16 feet
      longMid: 38,     // NBA average ~38% from 16-23 feet
      corner3: 37,     // NBA average corner 3 typically higher
      three: 36,       // NBA average ~35-36% from three
      deepThree: 25,   // Decreases with distance
      halfCourt: 15,   // Rarely attempted
      fullCourt: 5
    },
    'lebron': {
      name: 'LeBron James',
      restricted: 74, // 71% career at rim, elite finisher
      paint: 56,      // Dominant in paint scoring
      midRange: 39,   // 37.8% career mid-range
      longMid: 38,    // Consistent from long 2s
      corner3: 41,    // Strong corner three shooter
      three: 35,      // 34.9% career three-point
      deepThree: 28,  // Can hit deep shots
      halfCourt: 18,  // Better than average from distance
      fullCourt: 6
    },
    'jordan': {
      name: 'Michael Jordan',
      restricted: 74, // 74% at rim in prime (89-92)
      paint: 59,      // 59% in paint overall
      midRange: 51,   // 51.1% mid-range (legendary)
      longMid: 50,    // 49.5% from 16-3PT line
      corner3: 38,    // Decent corner three
      three: 33,      // 32.7% career three-point
      deepThree: 24,  // Not his strength
      halfCourt: 12,  // Rarely attempted
      fullCourt: 4
    },
    'shaq': {
      name: "Shaquille O'Neal",
      restricted: 73, // Led league in paint scoring 10 years
      paint: 61,      // 65.9% within 5 feet, 58.2% career FG%
      midRange: 32,   // Limited mid-range game
      longMid: 22,    // Rarely shot from here
      corner3: 8,     // Almost never attempted
      three: 5,       // Career long-range struggles
      deepThree: 3,   // Essentially impossible
      halfCourt: 2,   // Would never attempt
      fullCourt: 1
    },
    'kobe': {
      name: 'Kobe Bryant',
      restricted: 65, // Solid but not elite at rim
      paint: 49,      // Good paint scorer
      midRange: 44,   // 44% from 10-16 feet
      longMid: 40,    // Strong from long mid-range
      corner3: 37,    // Capable corner three shooter
      three: 34,      // 32.9% career, better in prime
      deepThree: 27,  // Would take and make tough shots
      halfCourt: 16,  // Mamba mentality
      fullCourt: 6
    },
    'magic': {
      name: 'Magic Johnson',
      restricted: 68, // Strong finisher at rim for his size
      paint: 52,      // 52% FG%, 54.1% on 2-pointers
      midRange: 42,   // Solid mid-range game
      longMid: 40,    // Capable from distance
      corner3: 32,    // 30.3% career 3PT%
      three: 30,      // Not a strong three-point shooter
      deepThree: 20,  // Rarely shot from deep
      halfCourt: 10,  // Occasionally took them
      fullCourt: 3
    },
    'curry': {
      name: 'Stephen Curry',
      restricted: 64, // 64% career restricted area
      paint: 52,      // 51-53% in paint outside restricted
      midRange: 45,   // Exceptional mid-range shooter
      longMid: 42,    // Strong from long 2s
      corner3: 45,    // Elite corner three
      three: 43,      // 42.6% career 3PT (all-time great)
      deepThree: 40,  // Best deep shooter ever
      halfCourt: 28,  // Makes half-court shots regularly
      fullCourt: 15
    },
    'davis': {
      name: 'Anthony Davis',
      restricted: 72, // Elite rim finisher, 75%+ in restricted
      paint: 60,      // 60-62% career in paint
      midRange: 42,   // 42% from mid-range
      longMid: 38,    // Capable from long 2s
      corner3: 33,    // Limited corner three
      three: 30,      // 29.7% career 3PT
      deepThree: 18,  // Rarely attempts deep shots
      halfCourt: 8,   // Almost never takes them
      fullCourt: 2
    },
    'durant': {
      name: 'Kevin Durant',
      restricted: 67, // Solid finisher
      paint: 55,      // Strong paint scorer
      midRange: 52,   // 52% mid-range (elite)
      longMid: 50,    // One of best long 2 shooters
      corner3: 43,    // Excellent corner shooter
      three: 39,      // 39% career 3PT
      deepThree: 35,  // Can hit from anywhere
      halfCourt: 20,  // Confident deep shooter
      fullCourt: 8
    },
    'luka': {
      name: 'Luka Dončić',
      restricted: 63, // Good finisher
      paint: 50,      // Solid paint game
      midRange: 43,   // Strong mid-range
      longMid: 40,    // Good from long 2s
      corner3: 37,    // Decent corner three
      three: 36,      // 35-38% career range
      deepThree: 30,  // Takes and makes step-back 3s
      halfCourt: 18,  // Confident from distance
      fullCourt: 7
    }
  };

  const calculateProbability = (x, y, basket, player) => {
    const profile = playerProfiles[player];
    ...
    if (distance < 3) {
      baseProb = profile.restricted;
      zone = "Restricted Area";
    }
    else if (distance < 8) {
      baseProb = profile.paint;
      zone = "Paint";
    }
    else if (distance < 16) {
      baseProb = profile.midRange;
      zone = "Mid-Range";
    }
    else if (distance < THREE_POINT_LINE) {
      baseProb = profile.longMid;
      zone = "Long Mid-Range";
    }
    else if (distance < 47) {
      const isCorner = Math.abs(distY) > 14 && distance < 28;
      if (isCorner) {
        baseProb = profile.corner3;
        zone = "Corner Three";
      } else if (distance < 28) {
        baseProb = profile.three;
        zone = "Three-Point";
      } else if (distance < 35) {
        baseProb = profile.deepThree;
        zone = "Deep Three";
      } else {
        baseProb = profile.halfCourt;
        zone = "Half Court Range";
      }
      
      if (distance > 28) {
        baseProb -= (distance - 28) * 0.5;
      }
    }
    else {
      baseProb = profile.fullCourt;
      zone = "Full Court";
      if (distance > 70) {
        baseProb = Math.max(1, baseProb - 2);
      }
    }
  ...
  useEffect(() => {
    if (shotPosition) {
      const result = calculateProbability(shotPosition.x, shotPosition.y, targetBasket, selectedPlayer);
      setProbability(result);
    }
  }, [shotPosition, targetBasket, selectedPlayer]);
        ...
        <div className="bg-slate-800 rounded-xl shadow-2xl p-6 border border-slate-700">
          ...
          {showInfo && (
            <div className="mb-6 p-4 bg-slate-700 rounded-lg text-slate-300 text-sm">
              ...
              <h3 className="font-bold text-white mt-3 mb-2">Player Profiles:</h3>
              <p className="text-slate-400 text-xs mb-2">Based on actual NBA career statistics</p>
              <ul className="space-y-1 text-xs">
                <li>• <strong>LeBron James:</strong> Elite inside scorer, solid three-point shooter</li>
                <li>• <strong>Michael Jordan:</strong> Legendary mid-range shooter, dominant scorer</li>
                <li>• <strong>Shaquille O'Neal:</strong> Unstoppable near basket, poor outside shooter</li>
                <li>• <strong>Kobe Bryant:</strong> Complete scorer, excellent mid-range game</li>
                <li>• <strong>Magic Johnson:</strong> Great finisher, strong paint scorer, weak 3PT</li>
                <li>• <strong>Stephen Curry:</strong> Greatest shooter ever, elite from everywhere</li>
                <li>• <strong>Anthony Davis:</strong> Elite rim protector/finisher, limited range</li>
                <li>• <strong>Kevin Durant:</strong> Lethal mid-range, great 3PT, complete scorer</li>
                <li>• <strong>Luka Dončić:</strong> Strong all-around, step-back specialist</li>
                <li>• <strong>Generic Player:</strong> NBA average shooting percentages</li>
              </ul>
